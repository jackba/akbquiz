...
public class BgMusic extends Service{
	private static String TAG = "BgMusic",regularExpression="<->"; 
	MediaPlayer player;
	AudioManager am;
	int PlayingNo=0;
    ...
    OnCompletionListener CompL= new MediaPlayer.OnCompletionListener(){
		...
		public void onCompletion(MediaPlayer mp) {
			...
			case MODE_LOOP:
				PlayingNo++;
				if(PlayingNo>=playlistLength)PlayingNo=0;
				break;
			case MODE_RANDOM:
			...
			try {
				if(playlistLength>0){
			        player=MediaPlayer.create(BgMusic.this, playlist[PlayingNo]);
			        if(isOn)player.start();
		        }else{
		        	player=MediaPlayer.create(BgMusic.this, Uri.parse("android.resource://"+ getPackageName() +"/"+ R.raw.bg));
		        }
    ...
    void errorToast(Exception e){
    	Toast.makeText(BgMusic.this, "Unexpected error in: "+TAG+" . error:"+e.getMessage(), Toast.LENGTH_SHORT).show();
    }
    OnErrorListener ErrL = new MediaPlayer.OnErrorListener(){
		@Override
		public boolean onError(MediaPlayer mp, int what, int extra) {
			Toast.makeText(BgMusic.this, "Unexpected media player error.", Toast.LENGTH_SHORT).show();
    ...
    public int onStartCommand(Intent intent,int flags, int startId) {  
        Log.d(TAG, "+onStart+");  
        isOn=intent.getBooleanExtra(Database.ColName_switch_bg, false);
        
        setPlaylist(intent.getStringExtra(Database.ColName_extend));
        am = (AudioManager) this.getSystemService(Context.AUDIO_SERVICE);
        am.setStreamVolume(AudioManager.STREAM_MUSIC, intent.getIntExtra(Database.ColName_vol_bg, 10),0);
        
        if(playlistLength>0){
	        player=MediaPlayer.create(this, playlist[PlayingNo]);
	        if(isOn)player.start();
        }else{
        	player=MediaPlayer.create(this, Uri.parse("android.resource://"+ getPackageName() +"/"+ R.raw.bg));
        }
    ...
    public class BGHandler extends Handler {
	    ...
	    public void handleMessage(Message msg) {
	        ...
	        case PLAYLIST_CHANGE:
	        	...
	        	PlayingNo=0;
	        	setPlaylist(bundle.getString(Database.ColName_extend));
